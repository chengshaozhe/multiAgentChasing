...
############
    ...
    wolfActionSpace = list(it.product(wolfActionOneSpace, wolfActionOneSpace))
    numWolfActionSpace = len(wolfActionSpace)
    ...
    depth = 5
    generateSheepModelSingle = GenerateModel(4, numSheepActionSpace, regularizationFactor)
    generateSheepModelMulti = GenerateModel(6, numSheepActionSpace, regularizationFactor)
    generateCenterControlWolf = GenerateModel(6, numWolfActionSpace, regularizationFactor)
    ...
    wolfDepth = 9
    initWolfNNModel = generateCenterControlWolf(sharedWidths * wolfDepth, actionLayerWidths, valueLayerWidths, resBlockSize, initializationMethod, dropoutRate)
    ...
    sheepPreTrainModelPathMulti = os.path.join('..', 'preTrainModel', 'agentId=0_iterationIndex=27_killzoneRadius=30_maxRunningSteps=100_numSimulations=200_numTrainStepEachIteration=3_numTrajectoriesPerIteration=16')

    wolfPreTrainModelPath = os.path.join('..', 'preTrainModel', 'agentId=1_depth=9_learningRate=0.0001_maxRunningSteps=100_miniBatchSize=256_numSimulations=200_trainSteps=50000')
    ...
    wolfPreTrainModelMulti = restoreVariables(initWolfNNModel, wolfPreTrainModelPath)
    ...
    wolfPolicy = ApproximatePolicy(wolfPreTrainModelMulti, wolfActionSpace)
    ...
    humanController = HumanController(writer, gridSize, stopwatchEvent, stopwatchUnit, wolfSpeedRatio, drawNewState, finishTime, stayInBoundary, saveImage, saveImageDir, beliefPolicy, wolfPolicy, chooseGreedyAction)
